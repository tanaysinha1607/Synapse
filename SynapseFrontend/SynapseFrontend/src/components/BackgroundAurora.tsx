...
export default function BackgroundAurora({ animationType = "rotate" }: { animationType?: AnimationType } = {}) {
  // Defaults (from your snippet)
  const height = 3.5;
  const baseWidth = 5.5;
  // animationType is provided via props; default is set in function signature
  const glow = 1;
  const offset = { x: 0, y: 0 };
  const noise = 0.5;
  const transparent = true;
  const scale = 1.0; // Reduced from 3.6 to show the entire prism in the background
  const hueShift = 0;
  ...
  useEffect(() => {
    ...
    const render = (t: number) => {
      ...
      if (animationType === "hover") {
        ...
        const prevRoll = roll;
        yaw = lerp(prevYaw, targetYaw, Math.max(0, Math.min(1, inertia || 0.12)));
        pitch = lerp(prevPitch, targetPitch, Math.max(0, Math.min(1, inertia || 0.12)));
